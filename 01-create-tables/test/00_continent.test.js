...
xdescribe('Continents Table', function () {
  beforeEach(function () {
  ...
  it('correctly rolls back the migration', function () {
    return knex.schema.hasTable('continents').then(beforeRollback => {
      return knex.migrate.rollback(this.config).then(() => {
        return knex.schema.hasTable('continents').then(afterRollback => {
