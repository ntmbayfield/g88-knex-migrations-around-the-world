...
xdescribe('Governments Table', function () {
  beforeEach(function () {
  ...
  it('creates the appropriate columns upon migration', function () {
    return knex('governments').columnInfo()
    .then((actual) => {
      const expected = {
        id: {
          type: 'integer',
          maxLength: null,
          nullable: false,
          defaultValue: 'nextval(\'governments_id_seq\'::regclass)'
        },
  ...
  it('correctly rolls back the migration', function () {
    return knex.schema.hasTable('governments').then(beforeRollback => {
      return knex.migrate.rollback(this.config).then(() => {
        return knex.schema.hasTable('governments').then(afterRollback => {
